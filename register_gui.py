...
class FuseGUI:
    def __init__(self):
        ...
        self.window = QWidget()
        self.window.setWindowTitle('图像配准')
        ...
        self.button_sr.clicked.connect(
            lambda: self.show_img(
                QFileDialog.getOpenFileName(self.window, '选择可见光图像', './', 'Image Files(*.jpg *.png)')[0],
                                  self.label_sr_image,self.label_ir_image, 0))
        ...
        # add a button to choose origin image
        self.button_fuse = QPushButton('配准', self.window)
        self.set_button(self.button_fuse, '配准', 1000, 400, 150, 30)
        ...
        self.set_text_label(self.label_result, '配准图像', 1000, 0, 150, 100)
    ...
    def show_img(self, img_path, label1, label2, flag, size=(320, 256)):
