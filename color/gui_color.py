...
class GUI:
    def __init__(self):
        ...
        self.set_button(self.button_fuse, '着色', 700, 400, 150, 30)
        self.button_fuse.clicked.connect(lambda: self.show_result(self.label_color_image, 0))

        # add a label
        self.label_origin = QLabel(self.window)
        self.set_text_label(self.label_origin, '待增强图像', 200, 0, 150, 100)

        # # add a label
        # self.label_ir = QLabel(self.window)
        # self.set_text_label(self.label_ir, '超分结果', 600, 0, 150, 100)

        # add a label
        self.label_result = QLabel(self.window)
        self.set_text_label(self.label_result, '着色结果', 700, 0, 200, 100)
